...
⁴ rebus is the demo page for **Rebus** (`code-typography/rebus`, GitHub
[`afcondon/purescript-rebus`](https://github.com/afcondon/purescript-rebus)), the
content → coloured-icon identity library. First published 2026-09-14 via
`quartermaster publish`; declaration at `code-typography/rebus/demo/compose.yml`.
It deploys from the library repo rather than a subdir here, like harmonia, because
the page is regenerated by `make demo` from the compiled library and a staged copy
would go stale into showing glyphs the library no longer mints. `cfProject` is
`hylograph-rebus` rather than the bare `rebus` the two most recent additions
(`liquid-purescript`, `signal-box`) would suggest: a Pages project name is also a
global `pages.dev` subdomain, so a short bare name squats one — and may already be
taken by another account, which fails the create.

**A trap for the next site published from here (2026-09-14):** wrangler 4.131
delegates `wrangler pages project create` to Workers, which looks for static
files in the *current working directory*, finds none when the command is run
from tooling, and aborts without creating anything — so `quartermaster publish`
can no longer create a Pages project that does not already exist. Create it once
by hand with `--force` (`npx wrangler pages project create <cfProject>
--production-branch main --force`), then let quartermaster do the rest; wrangler
states `--force` is needed only for that first create and must not be passed
afterwards. Quartermaster's emitted command has not been fixed for this — it is a
two-runtime change (node + the Gnomon Go twin) behind `make conformance`.
